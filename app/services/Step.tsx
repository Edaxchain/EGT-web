...
export default function InstallationSteps() {
    ...
    return (
        <section
            id="step"
            className="py-24 px-8 md:px-12 lg:px-48 bg-background"
        >
                ...
                <div className="relative">
                    ...
                    >
                        {steps.map((step) => (
                            ...
                            >
                                <Card className="relative overflow-hidden border-border bg-surface rounded-4xl transition-all duration-300 hover:shadow-xl group h-full flex flex-col w-full">
                                    {" "}
                                    <CardHeader className="pb-4 shrink-0">
                                        {/* Icon Badge */}
                                        <div
                                            className={`w-12 h-12 ${step.color} text-text-main rounded-2xl flex items-center justify-center mb-4 shadow-md transition-transform group-hover:scale-110`}
                                        >
                                            {step.icon}
                                        </div>
                                        <div className="space-y-1">
                                            <span className="text-xs font-bold uppercase tracking-[0.2em] text-brand-dominant">
                                                Step 0{step.id}
                                            </span>
                                    ...
                                    <CardContent className="flex-grow">
                                        <ul className="space-y-3">
                                            {step.details.map((detail, idx) => (
                                                <li
                                                    key={idx}
                                                    className="text-[13px] text-text-main font-light leading-snug flex gap-3"
                                                >
